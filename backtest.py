...
def annual_return(date_line, capital_line):
    ...
    #flash('年化收益率为：%f' % annual)
    return ("annual return:%f"  % annual)
...
def max_drawdown(date_line, capital_line):
    ...
    flash('最大回撤为：%f, 开始日期：%s, 结束日期：%s' % (max_dd, start_date, end_date))
...
def max_successive_up(date_line, return_line):
    ...
    flash('最大连续上涨天数为：%d  最大连续下跌天数为：%d' % (max_successive_up, max_successive_down))
...
def max_period_return(date_line, return_line):
    ...
    flash('最大单周期涨幅为：%f  最大单周期跌幅为：%f' % (max_return, min_return))
